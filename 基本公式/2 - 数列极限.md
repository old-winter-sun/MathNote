...
定义：$\displaystyle\lim_{n\to+\infty}a_n=a\Leftrightarrow\forall\epsilon>0,\enspace\exist{N}\in\N_+,\enspace当n>N时,\enspace恒有|a_n-a|<\epsilon$
- $\forall\epsilon>0$：无限逼近但不等于0的概念&nbsp;$\epsilon$
- $\exist{N}\in\N_+,\enspace当n>N时$：在数轴的右侧（无穷大方向），一直能找得到一个界限&nbsp;$N$，当&nbsp;$n$&nbsp;超过该界限时（比&nbsp;$N$&nbsp;更大）
